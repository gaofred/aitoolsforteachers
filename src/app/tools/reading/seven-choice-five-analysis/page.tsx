...
export default function SevenChoiceFiveAnalysisPage() {
  const router = useRouter()
  const { currentUser, userPoints, refreshUser } = useUser()

  // 调试日志：页面加载检查
  console.log('🔍 七选五解析页面加载完成, currentUser:', currentUser ? '已登录' : '未登录');
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await fetch('/api/ai/seven-choice-five-analysis', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">七选五解析工具</h1>
                <p className="text-sm text-gray-500">专业的七选五AI分析工具（Gemini-2.5-Pro模型驱动）</p>
              </div>
            </div>
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="在此粘贴七选五题目内容，或使用火山引擎豆包模型拍照识别图片中的文字...
                  ...
                  <div className="text-center space-y-6">
                    ...
                    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 max-w-sm mx-auto">
                      <div className="flex items-center space-x-2 text-indigo-700 text-sm">
                        ...
                        <span>AI正在分析七选五题目，生成专业的七选五解析内容</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
