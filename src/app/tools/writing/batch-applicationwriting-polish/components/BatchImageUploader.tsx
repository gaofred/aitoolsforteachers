...
const BatchImageUploader: React.FC<BatchImageUploaderProps> = ({
  ...
  const processAllImages = async () => {
    ...
    const message = `AI超级并行处理中... 预计${uploadedImages.length}张图片大约需要${estimatedMinutes}分钟（${Math.min(35, uploadedImages.length)}张同时处理，极速性能模式）。`;
    console.log(`🎯 ${message}`);
    ...
    console.log(`📝 开始作文批量处理 ${uploadedImages.length} 张图片，超级并发数: ${batchSize} 张/批次（极速OCR版 - 35张并发）`);
