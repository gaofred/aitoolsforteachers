  ...
  const recognizeText = async (images: string[]) => {
    ...
    try {
      ...
      const recognitionPromises = imageSnapshot.map(async (imageBase64, index) => {
        try {
          setRecognitionMessage(`火山引擎正在识别第${index + 1}张图片...`);
          ...
          if (!response.ok) {
            throw new Error(`火山引擎OCR请求失败: ${response.status}`);
          }

          const result = await response.json();

          if (!result.success) {
            throw new Error(result.error || '火山引擎OCR识别失败');
          }

          return {
            index,
            text: result.result || '',
            confidence: 1.0, // 火山引擎API不返回置信度，设为默认值
            provider: result.provider || '火山引擎'
          };
        } catch (error) {
      ...
      if (combinedText) {
        ...
        if (errors.length > 0) {
          alert(`火山引擎OCR识别失败：${errors.map(e => e.error).join(', ')}`);
        } else {
          alert('火山引擎OCR未能识别出有效文字，请检查图片质量或重新拍摄');
        }
      }
    } catch (error) {
      console.error('火山引擎批量识别失败:', error);
      alert('火山引擎图片识别失败，请重试');
    } finally {
