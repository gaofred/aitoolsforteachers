...
const ContinuationWritingResultTable: React.FC<ContinuationWritingResultTableProps> = ({
  ...
  const exportAnalysisToWord = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `学生读后续写共性问题分析报告_${new Date().toLocaleDateString('zh-CN').replace(/\//g, '-')}.docx`;
    ...
    try {
      ...
      const promises = completedAssignments.map(async (assignment) => {
        ...
        try {
          ...
          // 生成buffer并添加到ZIP
          const buffer = await Packer.toBuffer(doc);
          const fileName = `${studentName}_读后续写批改报告.docx`;
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="font-semibold text-blue-900">读后续写题目</h3>
                    </div>
