...
const DMXAPI_API_KEY = process.env.DMXAPI_KEY;
...
export async function POST(request: Request) {
  try {
    ...
    console.log('🌐 开始调用DMXAPI doubao-seed-1-6-flash-250828模型...');
    ...
    try {
      ...
      console.log('🔍 请求头配置:', {
        url: DMXAPI_URL,
        hasApiKey: !!DMXAPI_API_KEY,
        apiKeyLength: DMXAPI_API_KEY?.length,
        headers: Object.keys(headers)
      });

      // 调用DMXAPI进行OCR识别
      const ocrResponse = await fetch(DMXAPI_URL, {
        method: "POST",
        headers: headers,
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "你是一个专业的OCR识别助手，擅长准确识别图片中的文字内容。"
            },
            {
              role: "user",
              content: [
      ...
      try {
        ...
        console.log('🔍 DMXAPI API原始响应前500字符:', responseText.substring(0, 500));
      ...
      console.log(`🌐 DMXAPI API响应完成，总耗时: ${networkLatency}ms (${(networkLatency/1000).toFixed(2)}秒)`);

      if (!ocrResponse.ok) {
        console.error("❌ DMXAPI HTTP错误:", ocrData);
        return NextResponse.json({
          success: false,
          error: `DMXAPI HTTP错误 (${ocrResponse.status}): ${ocrData.error?.message || "HTTP请求失败"}`,
          details: {
      ...
      console.error('❌ DMXAPI网络请求失败:', networkError);
