  ...
  const response = await fetch(DASHSCOPE_API_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `你是一位严格的高中英语教师，专门负责高考读后续写评分。你必须严格按照给定的5档评分标准进行打分，不得放宽标准。

**重要评分原则**：
1. 严格执行档位要求，不得随意提高档次
2. 每个档位的分数范围是硬性规定：
   - 第五档：23-25分（必须达到所有要求才能给23分以上）
   - 第四档：18-22分（有明显缺陷不得进入此档）
   - 第三档：14-17分（基础水平，不得随意加分）
   - 第二档：6-13分（重大问题必须降档）
   - 第一档：1-5分（严重问题）

3. **严格扣分项**：
   - 偏离正确情节走向：立即降至第二档或第一档（10分以下）
   - 只写一段：最高13分
   - 词数不足150字：扣5分
   - 未使用规定首句：最多13分

4. **档位判定必须严格**：
   - 第五档要求：丰富合理内容+有效衔接+多样词汇语法+5个以上关键词
   - 第四档要求：比较丰富内容+比较有效衔接+比较多样词汇+4个以上关键词
   - 缺少任何一项都必须降档

${useMediumStandard ? '采用中等标准，严格按照评分标准打分，不额外宽容加分' : '严格执行高考评分标准，不因学生身份而放宽要求'}

请鼓励和保留学生正确使用的高级词汇，但错误使用必须扣分。`
        },
        {
          role: 'user',
          content: prompt
...
注意：学生作文的两个段落必须严格按照上述规定的首句开始续写，如果没有使用规定的首句，属于偏离题目要求，偏题题目要求，最多打13分（还要根据语法、词汇的表现进一步扣分）。
...
## 严格评分要求（必须遵守）
**⚠️ 重要：你是高考阅卷老师，必须严格执行评分标准，不得放宽要求！**

1. **档位判定必须严格**：
   - 只有完全达到第五档所有要求才能给23分以上
   - 任何明显缺陷都必须降档
   - 不得因为"鼓励"而随意加分

2. **基于原作评分原则**：
   - **评分必须基于学生提交的原作质量**
   - **不能因为后续修改或升格范文而给原作打高分**
   - **严格按学生实际表现的语言、情节、结构水平给分**
...
4. **档位要求检查清单**：
   - 第五档(23-25分)：丰富内容+有效衔接+多样词汇+5个关键词+无重大缺陷
   - 第四档(18-22分)：比较丰富内容+比较有效衔接+比较多样词汇+4个关键词+轻微缺陷
   - 第三档(14-17分)：基本合理内容+基本有效衔接+简单词汇+3个关键词+明显缺陷
   - 缺少任何必要元素必须降档
...
三、要按照我上面给你的评分标准以及下方的##打分定档，从答题要点、情节上合理、语言表达的地道性、单词拼写错误等方面依据下方的##打分定档情节简单，与要求有严重偏离且句式都是简单句，10分以下打分 进行定档给分。##读后续写词数统计  学生英语单词数少于150的，原基础上扣-5分。
...
但是，现在先不要定档打分，你明白了吗？Step by step,务必注意以上内容，综合考虑， step by step, 输出结果前，请仔细想一想，好了。Wait 现在，你可以先定档，再打分了，注意：

**⚠️ 重要评分原则**：
1. **必须基于学生原作质量打分**，不能因为后续修改而给原作高分
2. 情节走向正确只是基本要求，不是给高分的充分条件
3. 语言表达、词汇语法、篇章结构等都要严格按标准评分
4. 严重偏题（如完全偏离情节）必须按第一档处理（10分以下）
5. 不得因为"鼓励学生"或"修改后质量好"而给原作打高分

##读后续写词数统计  学生英语单词数少于150的，原基础上扣-5分。注意，务必输出学生的作文定档，以及学生获得的具体的分数。输出格式为##学生姓名+ 学生分数
${plotAnalysis ? `
##正确情节走向
${plotAnalysis}` : ''}
...
五、**重点检查：时态正确性分析**
读后续写讲述的都是过去发生的故事，必须使用过去时态。请逐一检查学生的时态使用：
- **时态要求**：必须使用一般过去时(Simple Past)、过去完成时(Past Perfect)、过去将来时(would do)、过去进行时(Past Continuous)等过去相关时态
- **常见错误**：
  - 一般现在时误用：用"am/is/are"代替"was/were"
  - 现在完成时误用：用"have/has done"代替"had done"
  - 一般将来时误用：用"will do"代替"would do"
- **检查方法**：逐句分析时态是否符合过去故事叙述的要求
- **输出要求**：列出所有时态错误，并给出正确的时态形式

六、指出学生作文中的所有语法、单词拼写错误，尽量指出15处或以上（连字符错误，标题符号错误，首句第一个大写字母未大写不计入，均不纳入考虑范围之内。）注意，语言不地道不算错误，连字符错误，标题符号错误，首句第一个大写字母未大写不计入，都请不要列出。明确列出具体错误10处（请标明是第一段还是第二段）。

七、以AI专家的口吻，针对学生原本的作文，写个中肯的作文评价，以"你"称呼"，汉字表达，300个汉字以内。

八、现在，请精确计算学生的作文，一共有几句话。请再呈现并逐句逐句输出学生原文句子，然后给出你的改动升级的句子版本，注意，呈现的都是完整的句子，像这样的完整的句子"Nowdays, some students did something fake when monthly paper"，并陈述你改动的理由。

**重要**：在每个句子的修改理由中，必须检查时态是否符合过去故事叙述的要求，如果有时态错误，必须明确指出并修正。你的输出的参考格式如下： **原文**: "Let's creat an honest test environment"
   **修改**: "Let us jointly uphold examination ethics through practical actions"
   **理由**: 修正拼写错误（creat→create），补充具体行动指南。

九、充分参照你上述你所提出的针对学生错误提供的逐句改动升级的句子版本【注意，两个段落的第一句话，均不做调整修改】，参照## 修改标准，输出你的润色好的版本。注意要符合#修改标准里的要求【范文为两段】(此时，输出"升格范文：学生的姓名），如果有逻辑上的不足，请你补上其他相关的句子，使逻辑上更加正确。使文章达到地道和满分的水准（欧洲语言标准B1-B2级别）。注意，词数必须超过180。注意：生成的范文需满足##正确情节走向的要求，不要写偏题了！然后，给出润色的作文版本，对应的中文翻译。
