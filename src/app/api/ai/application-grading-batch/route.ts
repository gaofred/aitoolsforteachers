...
2.注意以下几点：注意，单词错误多（比如多于10处才算多），语法错误多影响表达（但不影响理解的前提下可宽容），句式都是简单句（句式过于简单的），或字数不足（如少于80个单词），满足以上条件之一的，9分以下打分。##应用文词数 中的"英语单词数"少于60的，视为字数严重不足，按最高第一档（1-3分）打分。注意，先不要打分，wait, step by step.
3 现在，请给学生打分要按照我上面给你的打分标准，从答题要点、逻辑性、语言表达的地道性、单词拼写错误等方面给出合理的分数，注意，语言表达方向的打分要充分参照我给你的#学生范文（13分版本）##应用文词数中的"英语单词数"少于80的，要扣3分。**重要提醒：针对中国高中生，请适当宽容1分，不要因为一些小的语法或用词错误就过度扣分**。最后，输出姓名，输出你的修改后的分数。最后的分数输出格式为：##学生姓名+  打分：XX分。
4 按照## 评分标准| 你的提供范文的标准生成高分范文。
5 **特别说明：词汇处理原则** - 对于学生使用的高级词汇，只要语法正确，都应该予以保留和鼓励，不要为了追求"看起来更自然"而将高级词汇改为简单表达。中国式英语考试更注重词汇的丰富性和高级性。
6 **亮点词汇和句式表扬** - 请明确列出学生作文中的亮点词汇和句式表达，并给予表扬。格式如下：
## 亮点词汇与句式
**值得表扬的词汇**：[列出具体的高级词汇]
**精彩句式**：[列出学生使用的优秀句式结构]
