...
DO $$
BEGIN
    -- 删除 point_transactions 表的触发器
    IF EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_schema = 'public' AND event_object_table = 'point_transactions') THEN
        DROP TRIGGER IF EXISTS trigger_reset_points_after_transaction ON public.point_transactions;
        RAISE NOTICE '已删除 point_transactions 触发器';
    END IF;

    -- 删除 membership_purchases 表的触发器
    IF EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_schema = 'public' AND event_object_table = 'membership_purchases') THEN
        DROP TRIGGER IF EXISTS trigger_update_points_after_membership ON public.membership_purchases;
        RAISE NOTICE '已删除 membership_purchases 触发器';
    END IF;

    -- 删除 user_points 表的触发器
    IF EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_schema = 'public' AND event_object_table = 'user_points') THEN
        DROP TRIGGER IF EXISTS trigger_reset_points_daily ON public.user_points;
        RAISE NOTICE '已删除 user_points 触发器';
    END IF;
END $$;
