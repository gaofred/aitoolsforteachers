-- 终极PRO会员积分逻辑修复
- PRO会员：购买时+800积分，每天保持至少800积分
- PREMIUM会员：购买时+500积分，每天保持至少500积分
- 在Supabase SQL编辑器中执行此脚本

-- 1. 删除危险的批量重置函数
DROP FUNCTION IF EXISTS public.reset_all_daily_points() CASCADE;

-- 2. 修复会员购买函数 - 购买时添加奖励积分
...
BEGIN
    RAISE NOTICE '=========================================';
    RAISE NOTICE '终极PRO会员积分逻辑修复完成！';
    RAISE NOTICE '=========================================';
    RAISE NOTICE 'PRO会员逻辑：';
    RAISE NOTICE '- 购买时：300积分 → +800积分 = 1100积分';
    RAISE NOTICE '- 每天：保持至少800积分';
    RAISE NOTICE '  - 当前≥800积分：保持不变';
    RAISE NOTICE '  - 当前<800积分：补充到800积分';
    RAISE NOTICE '';
    RAISE NOTICE 'PREMIUM会员逻辑：';
    RAISE NOTICE '- 购买时：300积分 → +500积分 = 800积分';
...
SELECT
    ...
    CASE
        WHEN up.is_member = true AND up.daily_points >= 800 THEN 'PRO会员(保底800)'
        WHEN up.is_member = true AND up.daily_points >= 500 THEN 'PREMIUM会员(保底500)'
        ELSE '免费用户(保底25)'
    END as member_type
FROM users u
