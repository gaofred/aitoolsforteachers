...
BEGIN
    ...
    UPDATE user_points
    SET
        points = final_points,  -- 积分叠加
        daily_points = member_daily_points,
        is_member = true,
        membership_expires_at = new_end_date,  -- 时间叠加
...
BEGIN
    ...
    RAISE NOTICE '=========================================';
    RAISE NOTICE '修复内容：';
    RAISE NOTICE '1. 积分叠加：每次购买都增加对应积分';
    RAISE NOTICE '2. 时间叠加：续费在现有基础上累加天数';
    RAISE NOTICE '3. 智能判断：自动识别是新购买还是续费';
    RAISE NOTICE '';
    RAISE NOTICE '现在的逻辑：';
    RAISE NOTICE '- 第一次购买PRO：原积分300 + 800 = 1100积分，有效期365天';
    RAISE NOTICE '- 第二次购买PRO：原积分1100 + 800 = 1900积分，有效期730天';
    RAISE NOTICE '- 第三次购买PRO：原积分1900 + 800 = 2700积分，有效期1095天';
    RAISE NOTICE '=========================================';
END $$;
...
LEFT JOIN membership_purchases mp ON u.id = mp.user_id
WHERE up.is_member = true
GROUP BY u.id, u.email, up.points, up.daily_points, up.is_member, up.membership_expires_at
